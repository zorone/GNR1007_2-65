...
int fromDec(int num){
    ...
    char* pi_result = sprintf(pi_result,"%f", calcTemp);
    calcTemp = 180*temp/M_PI;
    char* a_result = sprintf(a_result,"%f", calcTemp);
    calcTemp = 180*temp/m_PI;
    char* m_result = sprintf(m_result,"%f", calcTemp);
    calcTemp = 180*temp/C_PI;
    char* c_result = sprintf(c_result,"%f", calcTemp);
    calcTemp = 180*temp/(F_PI);
    char* f_result = sprintf(f_result,"%f", calcTemp);
...
char* gcdFormat(int num1, int num2, int result){
    char *str = sprintf(str,"[%d, %d]", num1, num2);
    char *resFormat = sprintf(resFormat,"%d", result);
    str = sprintf("%[-11s] = %[-2s]", str, resFormat);
    return str;
}